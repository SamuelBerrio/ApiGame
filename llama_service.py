...
from llamaapi import LlamaAPI  # Asegúrate de que este módulo esté correctamente instalado y configurado
...
# Obtener el token de la API de Llama de las variables de entorno
...
# Inicializar el SDK de Llama de manera segura
...
def call_llama_api(prompt: str, max_tokens: int = 1000) -> Optional[str]:
    ...
    try:
        ...
        response.raise_for_status()  # Maneja respuestas HTTP de error
...
def generate_true_false_question(topic: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    content = call_llama_api(prompt, max_tokens=200)
    if not content:
...
def generate_multiple_choice_question(topic: str) -> Tuple[Optional[str], Optional[List[Tuple[str, str]]], Optional[str]]:
    ...
    content = call_llama_api(prompt, max_tokens=250)
    if not content:
...
def generate_comparison_question(topic: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    content = call_llama_api(prompt, max_tokens=500)
    if not content:
